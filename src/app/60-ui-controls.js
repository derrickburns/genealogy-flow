...
// winYrs / inferYrs slider handlers removed — see note on dwellWindow /
// inferenceYears constants above.
// Status filter (alive/deceased) was removed — person markers always
// represent people alive at the current year by definition. The
// fxCanvas fallback path still calls passesStatus / updateStatusLabels,
// so they remain as no-ops to avoid touching that legacy code path.
function updateStatusLabels(_yint) { /* no-op */ }
function passesStatus(_indi) { return true; }
...
function updatePanel(force) {
  ...
  if (inferenceYears > 0 && typeFilter.has(2)) for (let idx = 0; idx < lastIndividuals.length; idx++) {
  ...
  const html = rows.map(r => rowLiHtml(r, isRowExpanded(r))).join("");
  panelList.innerHTML = html || `<li style="color:#888;padding:14px;">No events visible.</li>`;
}

function isRowExpanded(r) {
  if (highlightedDwell < 0) return false;
  if (r.kind === "real") return r.i === highlightedDwell && highlightInferredYear < 0;
  return r.srcDwell === highlightedDwell && r.year === highlightInferredYear;
}
...
function rowInnerHtml(r) {
  if (r.kind === "real") {
    ...
    const c = sd === 0 ? [40,100,220] : sd === 1 ? [220,60,90] : [80,160,100];
    ...
    return `<span class="yr">${dwellY[i]}</span>${swatch}<span class="nm">${escHtml(ind?.name ?? "?")}</span>${relTag} <span class="ev">${EVENT_TYPE_LABEL[t] || "event"} ${escHtml(place)}</span>`;
  }
  ...
  const rel = relationCache.get(ind?.id);
  const relTag = rel ? `<span class="tag rel">${escHtml(rel)}</span>` : "";
  return `<span class="yr">${r.year}</span>${swatch}<span class="nm">${escHtml(ind?.name ?? "?")}</span>${relTag} <span class="ev">lived in ${escHtml(r.place)}<span class="tag">INFERRED ${r.basedOnYear}</span></span>`;
}

function rowContentHtml(r, expanded) {
  const inner = rowInnerHtml(r);
  if (!expanded) return inner;
  if (r.kind === "real") {
    const ind = lastIndividuals[dwellIndi[r.i]];
    if (!ind) return inner;
    return `<div class="liHead">${inner}</div>${renderExpandedCard(ind, r.i, false)}`;
  }
  const ind = lastIndividuals[r.indi];
  if (!ind) return inner;
  return `<div class="liHead">${inner}</div>${renderExpandedCard(ind, r.srcDwell, true)}`;
}

function rowAttrsHtml(r) {
  if (r.kind === "real") return `data-idx="${r.i}"`;
  return `data-idx="${r.srcDwell}" data-inferred="${r.year}" data-basedon="${r.basedOnYear}"`;
}

function rowClass(r, expanded) {
  if (r.kind === "real") return expanded ? "expanded selected" : "";
  return expanded ? "inferred expanded selected" : "inferred";
}

function rowLiHtml(r, expanded) {
  return `<li class="${rowClass(r, expanded)}" ${rowAttrsHtml(r)}>${rowContentHtml(r, expanded)}</li>`;
}

function liToRow(li) {
  const i = parseInt(li.dataset.idx, 10);
  if (Number.isNaN(i)) return null;
  if (li.dataset.inferred) {
    const year = parseInt(li.dataset.inferred, 10);
    const basedOnYear = parseInt(li.dataset.basedon, 10);
    const indi = dwellIndi[i];
    const ind = lastIndividuals[indi];
    if (!ind) return null;
    const sd = lastSideById ? (lastSideById.get(ind.id) ?? 2) : 2;
    const lastE = ind.events.find(e => e.year === basedOnYear);
    return { kind: "inferred", indi, srcDwell: i, year, place: lastE?.place ?? "", side: sd, basedOnYear };
  }
  return { kind: "real", i, year: dwellY[i] };
}

function findDwellForPersonYear(indId, year) {
  const ind = lastIndiById?.get(indId);
  if (!ind) return -1;
  const indiIdx = lastIndividuals.indexOf(ind);
  if (indiIdx < 0) return -1;
  const dwells = indiDwells.get(indiIdx);
  if (!dwells) return -1;
  let exact = -1, nearest = -1, nearestDist = Infinity;
  for (const di of dwells) {
    const dy = dwellY[di];
    if (dy === year) { exact = di; break; }
    const dist = Math.abs(dy - year);
    if (dist < nearestDist) { nearest = di; nearestDist = dist; }
  }
  return exact >= 0 ? exact : nearest;
}

function jumpToPersonYear(indId, year) {
  const di = findDwellForPersonYear(indId, year);
...
function applyExpansion() {
  const oldLi = panelList.querySelector("li.expanded");
  if (oldLi) {
    const oldRow = liToRow(oldLi);
    if (oldRow) {
      oldLi.className = rowClass(oldRow, false);
      oldLi.innerHTML = rowContentHtml(oldRow, false);
    }
  }
  if (highlightedDwell < 0) return;
  const wantInferred = highlightInferredYear >= 0;
  let newLi = null;
  if (wantInferred) {
    newLi = panelList.querySelector(`li[data-idx="${highlightedDwell}"][data-inferred="${highlightInferredYear}"]`);
  } else {
    const cands = panelList.querySelectorAll(`li[data-idx="${highlightedDwell}"]`);
    for (const li of cands) { if (!li.hasAttribute("data-inferred")) { newLi = li; break; } }
  }
  if (!newLi) {
    updatePanel(true);
    return;
  }
  const newRow = liToRow(newLi);
  if (!newRow) return;
  newLi.className = rowClass(newRow, true);
  newLi.innerHTML = rowContentHtml(newRow, true);
  newLi.scrollIntoView({ block: "start" });
}
  ...
  if (lastRootId && ind.id !== lastRootId) {
    ...
    if (path && path.length) {
      ...
      const fmtYears = (p) => {
        ...
        const yl = (y) => `<a class="yrlink" data-id="${escHtml(p.id)}" data-y="${y}">${y}</a>`;
        if (b && d) return ` <span class="yrs">${yl(b)}–${yl(d)}</span>`;
        if (b) return ` <span class="yrs">b. ${yl(b)}</span>`;
        if (d) return ` <span class="yrs">d. ${yl(d)}</span>`;
        return "";
      };
      ...
      chainHtml = `<div class="bcardSection"><h4>Lineage from root</h4><div class="bChain"><div class="step"><span class="nm" style="font-style:italic;color:#888;">No path through the family graph in this file. This person is unrelated by blood to the root.</span></div></div></div>`;
    }
  }
...
panelList.addEventListener("click", e => {
  const yrLink = e.target.closest(".yrlink[data-id][data-y]");
  if (yrLink) {
    e.preventDefault();
    const id = yrLink.dataset.id;
    const year = parseInt(yrLink.dataset.y, 10);
    if (!Number.isNaN(year)) { pushHistory(); jumpToPersonYear(id, year); }
    return;
  }
