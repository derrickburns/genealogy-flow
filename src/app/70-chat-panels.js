...
const _responsiveSheetHandleEl = $("responsiveSheetHandle");
const _responsiveSheetTabsEl = $("sideTabs");
const _responsiveSheetTitleEl = $("responsiveSheetTitle");
...
function _kfUpdateResponsiveSheetTitle(tab) {
  if (!_responsiveSheetTitleEl) return;
...
function _kfMarkMapTabActive() {
  if (!_kfUsesResponsiveShell()) return;
  _kfActiveSideTab = "map";
  if (typeof _kfSetLayoutUxState === "function") _kfSetLayoutUxState({ tab: "map" });
  _kfSyncSideTabChrome("map");
}

function _kfSyncResponsiveControlHeight() {
  const ui = $("ui");
  if (!ui) return;
  const tabs = $("sideTabs");
  const handle = $("responsiveSheetHandle");
  const tabsHeight = _kfUsesResponsiveShell() ? 0 : (tabs?.getBoundingClientRect().height || 0);
  const h = Math.ceil(
    ui.getBoundingClientRect().height +
    tabsHeight +
    (handle?.getBoundingClientRect().height || 0),
  ) + 8;
  document.documentElement.style.setProperty("--kf-responsive-ui-h", `${Math.max(118, h)}px`);
}

function _kfSetResponsiveSheetState(state) {
  const panel = $("panel");
  if (!panel) return;
...
function _kfPromoteResponsiveSheet() {
  const cur = $("panel")?.dataset.sheet || "peek";
  _kfSetResponsiveSheetState(cur === "peek" ? "open" : "full");
}

function _kfDemoteResponsiveSheet() {
  const cur = $("panel")?.dataset.sheet || "peek";
  _kfSetResponsiveSheetState(cur === "full" ? "open" : "peek");
}

function _kfRevealResponsiveSheetForTab(tab) {
  if (!_kfUsesResponsiveShell()) return;
  if (tab === "map") {
    _kfSetResponsiveSheetState("peek");
    return;
  }
  _kfSetResponsiveSheetState("open");
}

function _kfSetSideTab(tab) {
  let next = tab === "map" || tab === "person" || tab === "cluster" || tab === "trees" || tab === "tour" ? tab : "chat";
  if (next === "map" && !_kfUsesResponsiveShell()) next = "chat";
  if (next === "tour" && _kfDerivedCacheReady && typeof _kfShowYearTour === "function") _kfShowYearTour(false);
  if (_kfUsesResponsiveShell() && next !== "map" && playing) {
...
const _kfResponsiveUiResizeObserver = typeof ResizeObserver !== "undefined"
  ...
  : null;
if (_kfResponsiveUiResizeObserver && $("ui")) _kfResponsiveUiResizeObserver.observe($("ui"));

function _kfInstallResponsiveSheetDrag(handleEl, opts = {}) {
  if (!handleEl) return;
  let drag = null;
  ...
  handleEl.addEventListener("pointerup", e => {
    ...
    if (!moved) {
      if (ignoreTap) return;
      const cur = $("panel")?.dataset.sheet || "peek";
      if (cur === "peek") _kfSetResponsiveSheetState("open");
      else _kfSetResponsiveSheetState("peek");
    } else if (dy < -45) {
      _kfPromoteResponsiveSheet();
    } else if (dy > 45) {
      _kfDemoteResponsiveSheet();
    }
  });
  handleEl.addEventListener("keydown", e => {
    if (e.key === "Enter" || e.key === " ") {
      e.preventDefault();
      const cur = $("panel")?.dataset.sheet || "peek";
      _kfSetResponsiveSheetState(cur === "peek" ? "open" : "peek");
...
function _kfShowMapFromArtifact() {
  ...
  if (typeof _kfSetSideTab === "function" && _kfUsesResponsiveShell()) _kfSetSideTab("map");
}
...
function _kfRefreshChatScope(force = false) {
  ...
  const primaryCount = _kfUsesResponsiveShell() ? 3 : 4;
